...
**Status:** Stages 1 and 2 implemented (2026-09-06) — `"local"`, `"direct"` and `"kubernetes"`, the `backingServices:` config section, the required TCP health check, the ATS export, and the named port map ([#233](https://github.com/flojon/aspire-servicesources/issues/233), designed in [2026-09-05-backing-service-named-port-map-design.md](2026-09-05-backing-service-named-port-map-design.md)); stage 3 (secrets) remains. Accepted. Revised 2026-08-22 against `main` at #62, which removed the `ServiceResource` facade and shipped `Configure<T>`/`As<T>`; the proposed `AddService(configure:)` parameter and `WaitFor` shim are withdrawn as a result, and `AddBackingService` is now the design's only new public surface. Revised again 2026-08-30, when the supposed guest-language gap turned out not to exist. Earlier questions were settled by prototype and against a `kind` cluster, and the three team decisions that remained were all made on 2026-08-30. See Revision Notes.
