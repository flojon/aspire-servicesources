...
The probe code deliberately does not live on this branch — it is throwaway and must not merge to
`main`. It is on branch `worktree-ats-callback-spike`: the probe methods in
`src/Aspire.Hosting.ServiceSources/AtsCallbackProbe.cs`, the probe AppHost in
`samples/DemoAppHostTypeScript/probe-apphost.mts.txt` (copy over `apphost.mts`).
