...
internal sealed class JavaKindOptions
{
    ...
    /// <see cref="JavaLocalResourceKind.Validate"/> (so a bad block is reported alongside every
    /// other service's failure, before anything reaches the app model) and
    /// <see cref="JavaLocalResourceKind.Resolve"/> — it is cheap and side-effect free, so parsing
    /// twice is preferable to smuggling state between the two calls.
    /// </summary>
    ...
    /// lexically rather than by comparing resolved paths because the checkout doesn't exist yet when
    /// <see cref="JavaLocalResourceKind.Validate"/> runs — and a purely lexical check is what makes
    /// this reportable there, alongside every other service's configuration failure. Both separators
    /// count regardless of platform, so a Windows-style value is still rejected on Linux/macOS.
    /// </summary>
