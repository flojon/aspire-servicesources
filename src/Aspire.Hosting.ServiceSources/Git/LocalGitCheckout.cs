...
internal static class LocalGitCheckout
{
    ...
    public static string ManagedRepoRoot(string appHostDirectory, string serviceName)
    {
        ...
        return Path.Combine(appHostDirectory, ".servicesources", "checkouts", serviceName);
    }
    ...
    /// separator count whatever the platform: this configuration is shared across a team, so a value
    /// that only escapes on Windows still escapes, and the verdict cannot depend on who reads the
    /// file.
    /// </para>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// Asked here rather than at one gate every name passes, because at this version there is only
    /// one place a name becomes a path: <see cref="ManagedRepoRoot"/>. The scratch directory
    /// <c>CloneIntoPlace</c> names after the service sits downstream of it, on the managed route
    /// alone. A later place that builds a path from a name needs its own call to this.
    /// </para>
    /// <para>
