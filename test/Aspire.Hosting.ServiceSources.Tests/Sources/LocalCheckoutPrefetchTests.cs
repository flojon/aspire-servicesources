...
public class LocalCheckoutPrefetchTests
{
    ...
    /// <summary>
    /// The prefetch enumerates on the <c>source</c> value, so it has to read that value the same
    /// way <c>AddService()</c> resolves it — case-insensitively. A service spelled <c>"Local"</c>
    /// used to be dropped from the set silently, and its clone then serialised on the
    /// <c>AddService()</c> thread instead of running with the others: no error, just a slower
    /// start (#167).
    /// </summary>
    [Fact]
    ...
    {
        var dir = CreateAppHostDirectory("orders", "billing");
        // Re-spell one service's source. The Barrier below is what makes this an assertion about
        // the prefetch rather than about cloning at all: "billing" is only ever cloned on the
        // prefetch's own thread, so if the filter drops it, "orders" waits alone and times out.
